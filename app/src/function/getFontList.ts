...
export async function getFontList(element: HTMLElement) {
...
.font-test-span{
    ...
    for (let dom of waitingCheckDomList) {
        // console.log("dom", dom.getAttribute("cn_name"), dom)
    ...
    element.removeChild(outterDiv)
    return [...installedFontEnNameList]
}
// "Microsoft YaHei","微软雅黑",Lucida,Verdana,"Hiragino Sans GB","STHeiti","WenQuanYi Micro Hei",SimSun,sans-serif,Serif

document.fonts.load("12px 微软雅0黑", "ß").then((event) => {
    console.log(event)
})

for (let item of document.fonts.values()) {
    console.log(item)
}
